  ...
  return <PortfolioMotion>
    <main className={styles.page}>
      ...
      <section id="top" className={`${styles.opening} ${styles.movement}`}>
        ...
        <div className={styles.openingFooter}><span>Visual Artist</span><span>Selected Works</span></div>
      </section>
      ...
      <section className={`${styles.practice} ${styles.movement} practice-section`}>
        <div className={`${styles.practiceCopy} practice-copy`}><p className={styles.eyebrow}>Materials and practice</p><h2>Material Studies</h2><nav className={styles.practiceNav} aria-label="Materials">{practiceWorks.map(({ medium }) => <a href={`#practice-${medium.toLowerCase()}`} key={medium}>{medium}</a>)}</nav></div>
